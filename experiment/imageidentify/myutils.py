...
def getfour(cnts) -> list:
    ...
    result = []
    result.append(lefttopcnts)
    result.append(righttopcnts)
    result.append(rightbottomcnts)
    result.append(leftbottomcnts)
    return result


def getfours(cnts, imgw, imgh) -> list:
    """获取图像的四个顶点的轮廓点"""
    boundingBoxes = [cv2.boundingRect(c) for c in cnts]  # 用一个最小的矩形，把找到的形状包起来x,y,h,w
    rec = []
    reb = []
...
def getsigedian(cnts):
    refCnts = getfour(cnts)
    minx = sys.maxsize
    miny = sys.maxsize
    maxx = 0
    maxy = 0
